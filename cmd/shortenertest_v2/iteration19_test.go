...
// Iteration19Suite является сьютом с тестами и состоянием для инкремента
type Iteration19Suite struct {
	suite.Suite
}

// SetupSuite подготавливает необходимые зависимости
func (suite *Iteration19Suite) SetupSuite() {
	// check required flags
	suite.Require().NotEmpty(flagTargetSourcePath, "-source-path non-empty flag required")
}

// TestDocsComments пробует проверить налиция документационных комментариев в коде
func (suite *Iteration19Suite) TestDocsComments() {
	var reports []string
...
func (suite *Iteration19Suite) TestExamplePresence() {
	err := filepath.WalkDir(flagTargetSourcePath, func(path string, d fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
