...
func (suite *Iteration15Suite) serverUp(ctx context.Context, envs, args []string, port string) {
...
func (suite *Iteration15Suite) agentUp(ctx context.Context, envs, args []string, port string) {
...
func (suite *Iteration15Suite) serverShutdown() {
...
func (suite *Iteration15Suite) agentShutdown() {
...
func (suite *Iteration15Suite) TestCounterGzipHandlers() {
...
func (suite *Iteration15Suite) TestGaugeGzipHandlers() {
...
func (suite *Iteration15Suite) TestCollectAgentMetrics() {
	errRedirectBlocked := errors.New("HTTP redirect blocked")
	redirPolicy := resty.RedirectPolicyFunc(func(_ *http.Request, _ []*http.Request) error {
		return errRedirectBlocked
	})
	...
	req := httpc.R().
		SetHeader("Content-Type", "pallication/json")
...
cont:
	for ok := 0; ok != len(tests); {
		...
		for i, tt := range tests {
			...
			dumpErr := suite.Assert().NoError(err, "Ошибка при попытке сделать запрос с получением значения %s", tt.name)
