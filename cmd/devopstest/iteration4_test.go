...
func (suite *Iteration4Suite) TestCounterHandlers() {
	...
	suite.Run("update", func() {
		value1, value2 := rand.Int63(), rand.Int63()
		req := httpc.R().
		...
		dumpErr = dumpErr && suite.Assert().NotEqualf(nil, result.Delta,
			"Несоответствие отправленного значения counter (%d) полученному от сервера (nil), '%q %s'", value1+value2, req.Method, req.URL)

		dumpErr = dumpErr && suite.Assert().Equalf(value1+value2, *result.Delta,
			"Несоответствие отправленного значения counter (%d) полученному от сервера (%d), '%q %s'", value1+value2, *result.Delta, req.Method, req.URL)
...
func (suite *Iteration4Suite) TestGaugeHandlers() {
	...
	suite.Run("update", func() {
		...
		dumpErr = dumpErr && suite.Assert().NotEqualf(nil, result.Value,
			"Несоответствие отправленного значения counter (%f) полученному от сервера (nil), '%q %s'", value, req.Method, req.URL)

		dumpErr = dumpErr && suite.Assert().Equalf(value, *result.Value,
			"Несоответствие отправленного значения counter (%f) полученному от сервера (%f), '%q %s'", value, *result.Value, req.Method, req.URL)
