...
func (suite *Iteration14Suite) TestDeleteConcurrent() {
	...
	// делаем один проход синхронно - чтобы получить куку из запроса на создание шортурла
	// и перейти к асинхронной части теста уже с ней внутри клиента
	if err := urlProcessor(); err != nil {
		suite.T().Error(err)
	}
