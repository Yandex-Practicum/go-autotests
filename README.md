...
Если у вас возникают трудности с локальным запуском автотестов на компьютере Mac на базе процессора Apple Silicon (M1 и старше):

- убедитесь, что вы запускаете бинарный файл с суффиксом `darwin-arm64`
- у файла выставлен флаг на исполнение командой `chmod +x <filename>`
- вы разрешили выполнение неподписанного бинарного файл в разделе "Безопасность" настроек системы (см. ниже)

### Разрешение запуска неподписанного кода на Mac с процессором Apple Silicon

- Зайдите в раздел "Безопасность" настроек системы и найдите раздел в котором написано: "Использование <имя файла> было заблокировано, так как оно не от идентифицированного разработчика". Данный раздел появляется только после попытки запустить неподписанный бинарный файл.
- Нажмите на кнопку "Разрешить", чтобы внести бинарный файл в список разрешенных к запуску

![](.github/assets/m1-run-fix_1.png)

- Введите пароль или приложите палец к Touch ID для подтверждения разрешения

![](.github/assets/m1-run-fix_2.png)

Операция выполняется однократно для каждого бинарного файла, который вы хоитите запустить. Данную политику безопасности невозможно отключить на компьютерах Mac с процессором Apple Silicon.
